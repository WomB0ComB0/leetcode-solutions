...
class Solution {
   public:
    int maxScore(string s) {
        return *max_element(vector<int>(s.length() - 1).begin(), vector<int>(s.length() - 1).end(),
                            [&s](int i, int j) {
                                return (count(s.begin(), s.begin() + i + 1, '0') +
                                        count(s.begin() + i + 1, s.end(), '1')) >
                                       (count(s.begin(), s.begin() + j + 1, '0') +
                                        count(s.begin() + j + 1, s.end(), '1'));
                            });
    }
};
